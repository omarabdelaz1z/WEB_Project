...
$(document).ready(function () {
    $('#MessageSubjectTeam').submit(function (e) {
        e.preventDefault();
    });

    $('#send').click(function () {
        $("#send").prop('disabled', true).prop('value', 'Sending...').show("slow");
        let dataString = $('#MessageSubjectTeam').serialize();
        ...
        $.ajax({
            ...
            dataType : 'json',
            success : function (data) {
                $("#send").prop('disabled', false).prop('value', 'Send').show("slow");
                alert(data);
                window.location.reload();
            },
            error : function (data) {
